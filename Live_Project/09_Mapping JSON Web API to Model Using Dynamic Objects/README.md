...
This whole chunk of code may be eligible to become its own method, but because we only want to do this when we pull new API requests and we only want to do that every so often so as not to get locked out by the server I felt it was best to leave it within the _MeetUpApi method.
